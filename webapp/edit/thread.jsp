  	...
  	<table>
	  	<jspb:FolderTree>
				<tr>
					...
					<% } %>
					<td colspan="1">
						<a href='<%= context %>/update/thread?id=<%= thread.getField("id") %>&folder=<%= id %>'><img align="top" src="<%= context %>/images/closedfolder.gif"></a>
					</td>
					<td colspan="<%= Integer.parseInt(maxdepth)-Integer.parseInt(depth)+1 %>">
						<a class="closedfolder" href='<%= context %>/update/thread?id=<%= thread.getField("id") %>&folder=<%= id %>&redirect=/view/folder.jsp'><%= name %></a>
					</td>
				</tr>
	  	</jspb:FolderTree>
	  </table>
